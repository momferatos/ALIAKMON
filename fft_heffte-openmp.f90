...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    if(first) then
       ...
       !$acc data copyin(input( 1:size_in)) copyout(output(1:size_out)) create(work(1:size_work))
       !$acc host_data use_device(input( 1:size_in), output( 1:size_out), work( 1:size_work))
       call fft%forward(input(1:size_out), output(1:size_in), work(1:size_work))
       !$acc end host_data
       !$acc end data

       !$acc data copyout(input( 1:size_in)) copyin(output(1:size_out)) create(work(1:size_work))
       !$acc host_data use_device(input( 1:size_in), output(1:size_out), work( 1:size_work))
       call fft%backward(output(1:size_out), input(1:size_in), work(1:size_work))
       !$acc end host_data
       !$acc end data
    ...
    !$omp parallel num_threads(nthreads) proc_bind(spread) default(firstprivate) shared(fu, fft)



    nfi = omp_get_thread_num() + 1
#ifdef _CUFFT_
    !$omp critical
    cudaerror = cudasetdevice(int(mod(nfi - 1, numdevices), i4b))
    !$omp end critical
#endif
...
!!$omp do schedule(static, 1)
    ...
    if(dir == 1) then
       ...
       !$omp critical
#ifdef _CUFFT_
       call fft%forward(input(1:size_in),output(1:size_out),work(1:size_work), scale_cufft_none)
#elif defined _MKL_
       call fft%forward(input(1:size_in),output(1:size_out),work(1:size_work), scale_mkl_none)
#endif
       !$omp end critical
       !$acc end host_data
       !$acc end data
       ...
       !$omp critical
#ifdef _CUFFT_
       call fft%backward(output(1:size_out), input(1:size_in), work(1:size_work), scale_cufft_full)
#elif defined _MKL_
       call fft%backward(output(1:size_out), input(1:size_in), work(1:size_work), scale_mkl_full)
#endif
       !$omp end critical
       !$acc end host_data
       !$acc end data
