...
CXXFLAGS = -Mmkl $(MKLFLAGS) $(BUILDFLAGS) $(PARFLAGS)

FCFLAGS = -cpp $(MKLFLAGS) $(BUILDFLAGS) $(PARFLAGS)

FFTWROOT=$(LIBSROOT)/fftw
HEFFTEROOT=$(LIBSROOT)/heffte
CUDAROOT=$(CUDA_ROOT)
HDF5ROOT=$(LIBSROOT)/hdf5
HWLOCROOT=$(LIBSROOT)/hwloc
INCLUDE= -I $(FFTWROOT)/include -I $(HDF5ROOT)/include -I $(HEFFTEROOT)/include -I $(HWLOCROOT)/include -I $(CUDAROOT)/include 

LIB= -L $(FFTWROOT)/lib -L $(HDF5ROOT)/lib -L $(HEFFTEROOT)/lib -L $(HWLOCROOT)/lib -L $(CUDAROOT)/lib64 -L $(SZIPROOT)/lib

LDFLAGS =-lpthread -lm -ldl -lhdf5_hl -lhdf5hl_fortran -lhdf5_fortran -lhdf5 -lheffte -lhefftefftwfortran $(CUDALDFLAGS) $(MKLLDFLAGS) -lstdc++ -lmpi_cxx -lfftw3f -lfftw3f_mpi -lfftw3f_threads -lz -lhwloc -lcuda
